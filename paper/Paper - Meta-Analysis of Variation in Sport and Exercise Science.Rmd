...
The natural logarithm of the ratio of two means ($\textrm{ln}RR$) is also another effect size statistic that can be used [@curtis_meta-analysis_1998; @hedges_meta-analysis_1999; @lajeunesse_bias_2015]. The lnRR, and its sampling variance, $s^2_{lnRR}$ are given by:
...
In the context of *precision* or *personalised* approaches to exercise prescription comparison of variation between two groups, C and E, the $SD_{ir}$ has been proposed as an approach to determining the extent to which individual responses manifest [@hopkins_individual_2015; @atkinson_true_2015; @atkinson_issues_2019]. The standard deviation of changes scores (post-intervention scores minus pre-intervention scores) within E reflects the gross combination of a number of sources of variation including actual subject-by-intervention interactions (i.e., actual individual responsiveness), within-subject variability in intervention response (i.e., variability in response to the same intervention administered to the same subject), and random error (i.e., from pre and post measurements; @hecksteden_individual_2015). The standard deviation of changes scores from the C group by contrast only reflects random error [@hecksteden_individual_2015]. As such, the difference in these standard deviations can be used to determine the extent to which additional variation has been introduced by the intervention that might reflect individual responses. The $SD_{ir}$, and its sampling variance, $s^2_{SD_{ir}}$ are given by:
...
where $\textrm{ln}\hat\sigma$ is an estimate of $\textrm{ln}\sigma$, and it is assumed with sufficiently large sample size and value of $\sigma$ that $\textrm{ln}\sigma$ is normally distributed with variance $s^2_{\textrm{ln}\sigma}$. Given equations \@ref(eq:lnsigma-eq) and \@ref(eq:lnsigmavar-eq), the logarithm of the ratio of standard deviations of two groups such as C and E, the $\textrm{ln}VR$, and its sampling variance, $s^2_{\textrm{ln}VR}$ is given by: 
...
As noted, the examples presented used data from RT studies included in a recent meta-analysis published in the *Journal of Sport Sciences* [@polito_moderators_2021]. Here we have used their list of included studies and re-extracted data from 111 of these[^5]. All analysis examples were performed in using R (version 4.2.1, "Funny-Looking Kid", The R Foundation for Statistical Computing, 2022) using the **metafor** package [@viechtbauer_conducting_2010]. NOTE - ADD LINK TO ANALYSIS SCRIPT AND DATA

Polito et al. [-@polito_moderators_2021] conducted a systematic review and meta-analysis of randomised trials including a RT intervention where a non-training control group comparison group was included. Their analysis focused upon the SMD between the RT interventions and control groups from the studies included, with both overall effect estimate and moderator analyses performed. Given that Polito et al. [-@polito_moderators_2021] included only studies with non-training control groups, their study selection enables the examination of interindividual response variation by means of comparing the variances in change scores between the RT intervention and control groups. Table \@ref(tab:sample-size-tab) shows the total sample size, along with the median and range by group, across the included studies. Table \@ref(tab:summary-characteristics-tab).
...
knitr::kable(
  sample_sizes,
  caption = 'Sample sizes for resistance training (RT) and non training control (CON) groups for dataset.'
)
...
knitr::kable(
  summary_table,
  caption = 'Summary of study and participant characteristics (median [IQR], number of groups [%]).'
)
...
# Discussion
Given the apparent lack of awareness of the utility of meta-analytic frameworks for exploring variance, and the potential value such analyses can offer for the sport and exercise sciences, we have presented some existing effect size statistics and models pertinent to this that hopefully will encourage and support researchers in the field to embrace more than just the mean when engaging in quantitative evidence synthesis. The examples we presented used data from RT studies included in a recent meta-analysis published in the *Journal of Sport Sciences* [@polito_moderators_2021], and so hopefully are more relatable for researchers in sport and exercise sciences. 
